...
import { createFutureBooking, getBookingById, futureOpenCheckinDate } from '../../lib/ApiHelpers/BookingHelpers';
...
test.describe("booking/{id} PUT requests @booking", async () => {
  ...
  test(`Put a booking with a specific room id @happy`, async ({ request }) => {
    const putBody = {
      bookingid: bookingId,
      roomid: roomId,
      firstname: firstname,
  ...
  test("Put a booking without firstname in the request body", async ({ request }) => {
    const putBody = {
      bookingid: bookingId,
      roomid: roomId,
      lastname: lastname,
